...
<<<<<<< HEAD
En este caso sin embargo ponemos todo dentro de una funcion para correrlo con los distintos modelos seleccionados. Necesitamos entonces una funcion que ajuste el modelo en los datos de entrenamiento variando el valor de k y compare las predicciones con el dataset de testeo para hallar el k optimo. Cada funcion puede tener distintos niveles del valor k eso significa que tenemos que encontrar la combinacion de valores de k que llega al resultado optimo. Sin embargo la complejidad con la crece el espacio de parametros posibles a explorar es la longitud de la grilla de posibles valores de k elevado a la cantidad de covariadas que tenga el modelo. Sin embargo la mayoria de las variables que nos interesan no son continuas por lo que unicamente tenemos que estimar este parametro para 2 variables que son `release_year` y `runtime`. 
...
=======
df_grilla_k <- list(grilla_k) #lista con los valores posibles de k para una variable
df_grilla_k_2 <- tidyr::expand_grid(release_k = grilla_k, runtime_k = grilla_k) %>% as.list() #lista con los valores posibles de k para dos variables
>>>>>>> 3632466016695ce2cfe7bb0922a179ff8fed5f03
...
```{r}
gam(imdb_score ~ s(runtime, bs = 'cr'),
