...
=======
En este caso, sin embargo, ponemos todo dentro de una funcion para correrlo con los distintos modelos seleccionados. Necesitamos entonces una funcion que ajuste el modelo en los datos de entrenamiento variando el valor de k y compare las predicciones con el dataset de testeo para hallar el k optimo. Cada funcion puede tener distintos niveles del valor k por lo cual no necesariamente la combinacion optima de valores de k para modelos con mas de una variable es aquella donde k es igual en todos los casos. Sin embargo, la complejidad con la que crece el espacio de parametros posibles a explorar es la longitud de la grilla de posibles valores de k elevado a la cantidad de covariadas que tenga el modelo. Sin embargo, la mayoria de las variables que nos interesan no son continuas por lo que unicamente tenemos que estimar este parametro para 2 variables que son `release_year` y `runtime`. 

Para esto, vamos a armar una lista con todas las combinaciones posibles de 25 valores k entre 1 y 50.
>>>>>>> 3632466016695ce2cfe7bb0922a179ff8fed5f03
...
Asignamos como factor las variables que vayamos a usar en el GAM. 
...
Para asignar de manera automatica el valor k adecuado para cada modelo asi como si el valor corresponde a un solo k o a una combinacion de k. Para eso iteramos por todos los modelos y para cada caso nos vamos a fijar. Si tiene ".y" es que hay un dos k y tenemos que tomar segun el indice del primer y segundo elemento de la lista de posibles k que se le paso a la funcion `k_optimo_finder`, caso contrario tiene un solo k y se toma segun el indice del primer elemento de la lista. 
...
for(i in seq_along(modelos_candidatos)){
  ...
  k_optimo_por_modelo <- as.character(k_optimo_por_modelo)
  
  modelos_optimizados[[i]] <- ifelse(
    length(k_optimo_por_modelo) > 1,
    str_replace_all(modelos_candidatos[[i]],
                c('\\.x' = k_optimo_por_modelo [1], '\\.y' = k_optimo_por_modelo[2])
                ),
    str_replace(modelos_candidatos[[i]],
                '\\.x',
                k_optimo_por_modelo)
  )
}
...
tibble(
  rmse = rmse_todos_modelos %>% unlist(),
  cv_fold = rep(1:10, length(rmse_todos_modelos)),
  modelo = rep(modelos_optimizados %>% unlist(), each = 10)
) %>%
  ggplot(
    aes(x = cv_fold, y = rmse, color = modelo)
  ) +
...
```{r}
gam(imdb_score ~ s(runtime, bs = 'cr'), family = betar(link = 'logit'),
    data = gam_train %>% mutate(imdb_score = imdb_score/10)) %>%
  predict(gam_test) %>%
  tibble(predicciones = .) %>% 
    mutate(
      true_value = gam_test %>% mutate(imdb_score = imdb_score/10) %>% pull(imdb_score),
      error_cuadratico = (predicciones - true_value)^2
           ) %>%
    summarise(
      rmse = sqrt(mean(error_cuadratico, na.rm = T))
