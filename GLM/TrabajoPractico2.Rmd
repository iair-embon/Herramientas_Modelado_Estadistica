...
```{r}
comparacion_metodos <- function(formula){
  ...
  varios_modelos <- link_functions %>%
    map(
    ~ polr(formula, data = entrenamiento, method = .x)
  ) %>%
    map(
      ~ funcion_de_perdida(predict(.x, testeo), testeo$Q28)
    )
    
  
  # realizo predicciones en el df de testeo y uso la funcion de perdida del ejercicio 7
  funcion_de_perdida <- rep(NaN, length(varios_modelos))qa
