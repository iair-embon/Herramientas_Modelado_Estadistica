...
entrenamiento <- entrenamiento %>% filter(Q28 != 0) #Filtramos los datos con valor de respuesta 0
...
# corro el modelo
modelo_q28 <- polr(as.factor(Q28) ~ age, data = entrenamiento, method = "logistic")
summary(modelo_q28)
...
modelo_q9 <- polr( as.factor(Q9) ~ age, data = entrenamiento, method = "logistic")
summary(modelo_q9)
...
```{r}
funcion_perdida <- function(modelo, test_set, variable_respuesta){
...
```{r}
funcion_perdida(modelo_q28, testeo, testeo$Q28)
funcion_perdida(modelo_q9, testeo, testeo$Q9)
```
...
m3 <- lm(Q28 ~ age, data = entrenamiento)
summary(m3)

predicciones <- round(predict(m3, testeo))
```
...
# obtengo el resultado de la funcion de perdida en el modelo lineal
funcion_de_perdida_lineal <- sum(abs(as.numeric(testeo$Q28) - predicciones))/nrow(testeo)

# parece ser que el modelo lineal presenta una menor funcion de perdida que el modelo de regresion ordinal con cualquiera de sus 4 link functions. Por lo que se prefiere el modelo de regresion lineal para estos datos.
```
